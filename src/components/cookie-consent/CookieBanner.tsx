...
export function CookieBanner() {
  ...
  if (!isReady || (!showBanner && !settingsOpen)) return null;
  ...
  return (
    <div
      ...
      className="fixed inset-x-0 bottom-0 z-[100] border-t border-white/10 bg-[#0c0c12]/95 p-4 backdrop-blur-md sm:p-6 md:p-8"
    >
      <div className="mx-auto flex max-w-6xl flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        {!settingsOpen && (
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:gap-3 md:shrink-0">
            <button
              type="button"
              onClick={openSettings}
              className="w-full rounded-sm border border-white/10 px-5 py-2.5 text-sm text-[#9a958d] transition-colors hover:border-white/20 hover:text-[#f5f0e8] sm:w-auto"
            >
              Anpassa
            </button>
            <button
            ...
            <button
              type="button"
              onClick={acceptAll}
              className="w-full rounded-sm border border-[#e8b86d]/40 bg-[#e8b86d]/10 px-5 py-2.5 text-sm text-[#e8b86d] transition-colors hover:bg-[#e8b86d]/20 sm:w-auto"
            >
              Acceptera alla
            </button>
          </div>
